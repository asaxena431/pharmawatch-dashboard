...
def unique_name(source: str, extension: str = ".xml", moment: Optional[datetime] = None) -> str:
    ...
    The source's name stays legible, and a timestamp to the millisecond plus a
    short random suffix keep two runs of the same case apart.
    """
...
def load_config(path: str) -> ServiceConfig:
    ...
    return ServiceConfig(
        ...
        email=EmailSettings(
            ...
            subject_prefix=mail.get("subject_prefix") or "[cvm-aer]",
...
def _is_form(path: str) -> bool:
    ...
    try:
        return is_1932a_form(path) or is_1932_form(path)
...
def send_failure_email(config: ServiceConfig, archive: str, reason: str, attachment: Optional[str] = None) -> bool:
    ...
    try:
        if settings.transport == TRANSPORT_OUTLOOK:
            ...
            message["From"] = settings.sender or "cvm-aer@localhost"
...
def process_zip(archive: str, config: ServiceConfig) -> Processed:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="cvm-aer-case-") as workspace:
